...
public class WebAvatarLoader_gltfast : MonoBehaviour
{
    ...
    public async void LoadAvatar(string avatarUrl)
    {
        Debug.Log("LoadAvatar : " + avatarUrl);
        UISystem.Instance.SetLoading(true, uiPos);

        GameObject go = GameObject.Find("glTF");
        if (go)
        {
            Destroy(go);
        }

        var myModel = new GameObject("glTF");
        myModel.AddComponent<Animator>();

        var gltf = new GltfImport();
        var success = await gltf.Load(avatarUrl);

        if (success)
        {
            gltf.InstantiateMainScene(myModel.transform);

            OnAvatarLoaded(myModel);
        }
        else
        {
